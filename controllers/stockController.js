...
/** Port of recordStockTransaction() — validates OUT never exceeds currentStock, matching the original exactly. */
async function recordTransaction(req, res) {
  try {
    ...
    const item = await StockItem.findOne({ itemId }).lean();
    ...
    await logAudit({ user: req.user._id, action: type, entityType: "Stock", details: `${item.itemName} — ${qty} ${item.unit}` });
    ...
    res.redirect(`/stock?message=${encodeURIComponent(`Stock ${type} recorded successfully.`)}`);
  } catch (err) {
...
module.exports = { listStock, showNewForm, createItem, recordTransaction, itemTransactions };
